...
class _SubscriptionPageState extends State<SubscriptionPage> {
    ...
    if (userId != null) {
      ...
      if (controller.programDataList.isNotEmpty) {
        for (var program in controller.programDataList) {
          ...
          _programStartTimes[program.bookingId!] =
              _parseStartTime(program.date, program.time);
        }
      }
    } else {
  ...
  bool isProgramStarted(String date, String timeRange, int bookingId) {
    ...
    final endTime =
        _parseEndTime(date, timeRange); // Pass the 'date' to _parseEndTime
  ...
  DateTime _parseEndTime(String date, String timeRange) {
    try {
      final endTimeStr = timeRange
          .split('-')
          .last
          .trim(); // Extract the end time from the range
      final combined =
          "$date $endTimeStr"; // Combine the date with the end time
      final format =
          DateFormat("yyyy-MM-dd hh:mm a"); // Use appropriate format for time
      final parsed = format
          .parseStrict(combined)
          .toLocal(); // Parse to DateTime in local timezone
      print("✅ Parsed local end time: $parsed");
      return parsed;
    } catch (e) {
      print("❌ Failed to parse end time: $e");
      return DateTime.now()
          .add(Duration(days: 365)); // Return a far future date on error
    }
  }
  ...
  void _handleProgramAction(ProgramData program) async {
    ...
    if (program.isHost) {
      ...
      handleProgramStart(userId, program.bookingId!, program.time, program.date,
          program.programType!);
    } else {
      ...
      if (!isProgramStarted(program.date, program.time, program.bookingId!)) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content:
                Text('Host has not started the session yet. Please wait.')));
      } else {
              ...
              Expanded(
                child: Obx(() {
                  ...
                  filteredActive.sort((a, b) {
                    ...
                    return aStartTime.compareTo(
                        bStartTime); // Ascending order (earliest first)
                  });
                  ...
                  filteredExpired.sort((a, b) {
                    final aStartTime = _parseStartTime(a.date, a.time);
                    final bStartTime = _parseStartTime(b.date, b.time);
                    return bStartTime.compareTo(
                        aStartTime); // Descending order (newest first)
                  });
                  ...
                  return RefreshIndicator(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final isExpired =
                            isProgramExpired(program.date, program.time);
                        final isStarted = !isExpired &&
                            isProgramStarted(
                                program.date, program.time, program.bookingId!);
                        final hostJoined =
                            _hostHasJoined[program.bookingId] ?? false;
                        final restarting =
                            !program.isHost && isStarted && !hostJoined;
  ...
  void showStartTimePopup(String startTime, String date) {
    showDialog(
      context: context,
      barrierDismissible:
          false, // Prevent dismiss on outside tap or back button
      builder: (context) {
        return AlertDialog(
          title: Text(
      ...
      if (_sessionId != null) {
        ...
        if (restartResult != null) {
          ...
          if (restartResult['status_code'] == 200 ||
              (restartResult['status_code'] == null &&
                  responseData['channel_name'] != null)) {
            // Successful rejoin
