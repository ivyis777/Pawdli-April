...
import 'package:pawlli/presentation/screens/pet_store/addresslistpage.dart';
...
// Payment Method
  ...
  double _calculateItemSavings() {
    return cartController.cartItems
      ...
      final selling = double.tryParse(item.priceAtAdded ?? '0') ?? 0.0;
  ...
  void _calculateTotals() {
    ...
    // 2️⃣ Wallet balance
    double wallet =
        double.tryParse(walletBalanceController.walletBalanceAmount.value) ??
            0.0;
  ...
  Future<void> _onPaymentSuccess(PaymentSuccessResponse res) async {
    ...
    final verify = await RazorpayVerifyService.verifyPayment(
      RazorpayVerifyRequestModel(
        paymentTransactionId: checkoutRes.paymentTransactionId.toString(),
  ...
  void _startRazorpay({
    ...
    final razorpayOrder = await RazorpayPaymentService.createRazorpayOrder(
...
// ---------------- SELECTED CART ITEMS LIST ----------------
          ...
          SizedBox(
            ...
            child: Obx(() => ElevatedButton(
                  onPressed:
                      checkoutController.isLoading.value ? null : _onProceed,
  ...
  Widget paymentMethodSection() {
    ...
    final String codTitle =
        codEnabled ? "Cash on Delivery" : "Cash on Delivery (upto ₹500 only)";
