...
class _ProductDetailsScreenState extends State<ProductDetailsScreen> {
  ...
  Future<void> fetchProductVariants() async {
    ...
    try {
      ...
      final fetchedVariants = await ApiService.fetchVariantsList(productId);
  ...
  Future<void> addToCart() async {
    ...
    final existingItem = cartController.cartItems.firstWhereOrNull(
      (item) =>
          item.storeProduct == widget.product.storeproductId &&
          item.storeProductVariant == selectedVariant!.variantId,
    );
  ...
  void _shareProduct() async {
    ...
    final price = selectedVariant?.discountedPrice ??
        selectedVariant?.regularPrice ??
...
// ==========================
  ...
  Widget build(BuildContext context) {
    ...
    final int availableStock = selectedVariant?.availableStock ?? 0;
    ...
    final bool hasDiscount = discountedPriceValue > 0 &&
        regularPriceValue > 0 &&
    ...
    final int discountPercent =
        hasDiscount ? ((discountAmount / regularPriceValue) * 100).round() : 0;
    // ====================================================

    final String productName = product.productName ?? "Unnamed Product";

    final String? discountedPrice = product.discountedPrice;
    ...
    final List<String> productImages =
        product.productImages != null && product.productImages!.isNotEmpty
            ? product.productImages!.map((img) => img.toString()).toList()
            : [];

    final List<String> productTags = product.productFeatures ?? [''];
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Container(
        ...
        child: ElevatedButton.icon(
          ...
          onPressed: (isOutOfStock || isAddingToCart) ? null : addToCart,
                    ...
                    : variants.isEmpty
                        ...
                        : Wrap(
                            ...
                            children: variants.map((variant) {
                              final isSelected = selectedVariant?.variantId ==
                                  variant.variantId;
