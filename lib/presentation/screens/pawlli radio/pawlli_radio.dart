...
class _PawlliRadioState extends State<PawlliRadio> {
  ...
  final WalletBalanceController walletBalanceController =
      Get.put(WalletBalanceController());
  final ListenerPaythroghwalletController walletController =
  ...
  bool isProgramStarted(String date, String timeRange, int bookingId) {
    final start =
        _programStartTimes[bookingId] ?? _parseStartTime(date, timeRange);
    final end = _parseEndTime(date, timeRange);
  ...
  void _handlePaymentSuccess(PaymentSuccessResponse response) async {
    try {
      ...
      if (verification != null) {
        debugPrint(
            " Payment verification successful, navigating to success page.");
        ...
        debugPrint(
            "❌ Payment verification failed, navigating to failure page.");
  ...
  Future<void> createOrderThroughWallet({
    ...
    await Get.find<ListenerPaythroghwalletController>()
        .initiatelisternerPayment(
      amount: amount,
      currency: "INR",
      bookingId: bookingId,
      purpose: "Listener",
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      image: const DecorationImage(
                        image: NetworkImage(
                            "https://pawlli-podcasts.s3.ap-south-1.amazonaws.com/static_images/heartimage.png"),
                        fit: BoxFit.cover,
                      ),
                    ),
                  ),
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  ...
                                  SizedBox(
                                      width: 10), // Space between date and icon
                                  CircleAvatar(
                                    ...
                                    child: IconButton(
                                      ...
                                      onPressed: () {
                                        _selectDate(
                                            context); // Show date picker when clicked
                                      },
                                    ),
                                  )
                                ],
              ...
              Expanded(
                child: Container(
                  ...
                  child: Obx(() {
                    ...
                    final programList =
                        programController.programList.value?.data;
  ...
  songTile(
    ...
    String formattedLanguages = (languages is List<String>)
        ? languages.join(", ")
        : languages.toString();
  ...
  Future<void> handleJoinCall(int userId, int? sessionId) async {
    ...
    try {
      ...
      final uid = int.tryParse(agoraUidStr ?? '') ??
          0; // Agora allows 0 as a valid uid (SDK assigns)
